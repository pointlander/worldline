...
func MakeLoopsFFT3(N, Loops int) []Worldline {
	loops := make([]Worldline, Loops)
	rnd := rand.New(rand.NewSource(int64(1)))
	y := make([]complex128, 0, N*Loops*d)
	...
	yt := fft.FFTN(dsputils.MakeMatrix(y, []int{d, Loops, N}))
	...
	for loop := 0; loop < Loops; loop++ {
		for i := 0; i < N; i++ {
			...
			for j := 0; j < d; j++ {
				...
				point[j] = real(yt.Value([]int{j, loop, i})) / norm[loop][j]
			}
